        ...
        AdminAuthService.getRoleDetail({
            ...
            .then(response=>{
                ...
                for(let item of existing_pr){
                    const finded = this.permissionslists.filter(x=>x.id==item.permission_id)[0]
    ...
    methods: {
        onSubmit(){
            ...
            AdminAuthService.updateRole({
                ...
                .then(response=>{
                    NotifyService.commitNotify( { color: 'positive', message: 'Updated Successfully', icon: 'check', position: '' });
